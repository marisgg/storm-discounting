                ...
                for (uint64_t action = 0; action < pomdp->getNumberOfChoices(state); ++action) {
                    ...
                    if (rewardModel.hasTransitionRewards()) {
                        ...
                        STORM_LOG_THROW(false, storm::exceptions::NotImplementedException,
                                        "Transition rewards are currently not supported by the transformation!");
                    } else {
                        os << "            return ";
                        if (min) {
                            os << "-";
...
template void exportPomdpToJulia<double>(std::ostream& os, std::shared_ptr<storm::models::sparse::Pomdp<double>> pomdp, double discount,
                                         storm::logic::Formula const& formula);
template void exportPomdpToJulia<storm::RationalNumber>(std::ostream& os, std::shared_ptr<storm::models::sparse::Pomdp<storm::RationalNumber>> pomdp,
                                                        double discount, storm::logic::Formula const& formula);
}  // namespace exporter
}  // namespace pomdp
}  // namespace storm
